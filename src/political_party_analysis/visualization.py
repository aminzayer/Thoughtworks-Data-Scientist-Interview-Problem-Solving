...
def plot_finnish_parties(transformed_data: pd.DataFrame, splot: pyplot.subplot = None):
    ...
    for group in finnish_parties:
        # Filter data for the current group of parties
        ...
        if mask.any():
            ...
            # Create a label from the party names
            party_label = ', '.join(group["parties"])

            # Plot the filtered data
            scatter_plot(filtered_data, color=group["color"], splot=splot, label=party_label)
